...
try {
  ...
  for (const project of projects) await page(`work/${project.slug}`, `${project.focus} — Benji Kim`, await Project({params:Promise.resolve({slug:project.slug})}));
  ...
  console.log(`Built ${projects.length + 1} portfolio pages in pages-dist`);
} finally {
